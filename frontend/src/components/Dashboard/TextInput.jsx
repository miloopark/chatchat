  ...
  sendToChatGPT = async () => {
    ...
    try {
      ...
      if (!chatbotResponse.ok) {
        ...
        console.error(
          `Error from chatbot API: ${chatbotResponse.status} - ${errorDetails}`,
        );
        throw new Error(
          `Chatbot API responded with status: ${chatbotResponse.status}`,
        );
      }
      ...
      if (!storeUserResponse.ok) {
        ...
        console.error(
          `Error storing user message: ${storeUserResponse.status} - ${errorDetails}`,
        );
        throw new Error(
          `Error storing user message with status: ${storeUserResponse.status}`,
        );
      }
      ...
      if (!storeBotResponse.ok) {
        ...
        console.error(
          `Error storing bot response: ${storeBotResponse.status} - ${errorDetails}`,
        );
        throw new Error(
          `Error storing bot response with status: ${storeBotResponse.status}`,
        );
      }
    } catch (error) {
    ...
    return (
      <div className={fieldClassName}>
        ...
        <IconButton
          ...
          style={{ color: "#620062", backgroundColor: "white" }}
          className="mic-button"
        >
          <MicIcon />
        </IconButton>
        <IconButton
          onClick={this.toggleStopResponse}
          style={{ color: "#620062", backgroundColor: "white" }}
